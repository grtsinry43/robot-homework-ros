...
> Status: draft  ·  最后更新：2026-06-12  ·  本文档是架构与接口契约的单一真源。运维见 `CLAUDE.md`，进度见 `PROGRESS.md`。
...
### 1.3 通用性定位：任务组合通用，操作原语受限

本课设的"通用"体现在**组合层**而非原语层：面对任意自然语言指令，LLM 将其拆解为受限原语词汇表（pick / place + 语义 offset）上的计划，执行失败时依据结构化反馈**重规划**。操作原语本身（顶抓、固定爪宽）刻意保持简单——这是 SayCan / Code-as-Policies 一类系统的共同范式。

组合通用性由三块中间层支柱支撑（详见 §4.5）：

1. **Atomic Action Library**（§4.5.1）——机器人能力契约：声明式 JSON 描述每个原语的输入/输出/前置条件/错误码；能力扩展 = 改配置，不改 prompt。
2. **Robot Context + Scene Blackboard**（§4.5.2）——运行时观察与对象 grounding：把自然语言名词落到持久 `object_id`，把系统状态汇总成 LLM 可读的观察。
3. **Plan Executor + Structured Feedback**（§4.5.3）——计划校验与失败反馈：计划逐步执行、失败即停、返回逐步 Observation，使 LLM 能做针对性重规划而非盲目重试。

验收不用单条固定指令，而用一组考察不同拆解能力的场景（§3.5）。
...
前端只做透明化，不做控制逻辑。一屏三块内容：

```
用户输入：把红色方块放到盘子里

LLM 规划：pick_object(red_block_01) → place_at(blue_plate_01, above)

执行状态：成功 / 失败(code + reason) / 参数缺失 / 超出工作空间
```

- **用户输入**：文字框 + 语音开关（§3.1）
- **LLM 规划**：展示 LLM 产出的工具序列（`execute_plan` 的 plan_json，或逐步 ReAct 的 Action 流）
- **执行状态**：直接来自工具返回 JSON——`ok` / `failed`（展示 code + reason，§4.3）/ `aborted`；走 `execute_plan` 时按 `steps[]` 逐步刷新进度
- 实现形态不限（CLI 富文本 / 单页 Web 均可），验收只要求三块信息可见。

### 3.5 验收演示场景

通用性的证明方式是**同一套原语词汇表上的不同拆解**，验收用以下场景而不是单条固定指令：

| # | 指令示例 | 考察点 | 预期拆解 |
|---|---|---|---|
| S1 | "把红方块放到盘子里" | 基线 grounding | scan → pick → place(above) |
| S2 | "把所有方块都放进盘子里" | 对 scan 结果循环规划 | scan → (pick → place) × N |
| S3 | "交换红方块和绿方块的位置" | 无交换原语，需推理临时位置 | 红→临时位（left_of 某物）→ 绿→红原位 → 红→绿原位 |
| S4 | "把红方块叠到绿方块上" | place 目标可以是另一个方块 | scan → pick(红) → place_at(绿, above) |
| S5 | 执行中把目标物体移走/藏起 | **失败反馈 → 重规划**（核心） | `OBJECT_NOT_VISIBLE` → 重新 scan → 修补计划或报告用户 |
| S6 | 执行中说"停" | 抢占中断 | abort → `SERVO_ABORTED` → 询问是否继续 |

S5 是本课设工作量的核心论证点：结构化错误（§4.3）+ Plan Executor 的逐步失败反馈（§4.5.3）使 LLM 能做针对性重规划，而非盲目重试或人工兜底。
...
LLM 默认 prompt 里**只暴露前 7 个**。最后两个是 debug / 单测工具，prompt 明确禁止 LLM 调用。

| 工具 | 阻塞 | 典型耗时 | 用途 |
|---|---|---:|---|
| `get_action_library()` | 同步 | <0.1s | 返回原子能力库（声明式 JSON 契约，§4.5.1） |
| `get_robot_context()` | 同步 | <0.5s | 运行时观察：场景摘要 + readiness + last_error + recent_events（§4.5.2） |
| `execute_plan(plan_json)` | 同步 | 各步之和 | 校验并执行短线性计划，失败即停并返回逐步反馈（§4.5.3、D6） |
...
通用拆解能力由三块基础设施支撑（已实现于 `mcp_pick_place_brain.py` 与 `panda_pick_place/`）。

#### 4.5.1 Atomic Action Library：机器人能力契约

- `config/atomic_actions.json`：每个原语动作的声明式契约——`name / kind / inputs / outputs / preconditions / side_effects / errors / safety / implementation_refs`，加载时强校验（`errors` 必须属于 §4.3 错误码枚举，未知字段/重名直接拒绝）。
- LLM 通过 `get_action_library()` 查询，拿到结构化 JSON + 渲染后的能力目录文本。
- **能力扩展 = 新增 JSON 条目 + 对应实现，prompt 不用改**——这是"词汇表可声明式扩展"（§1.3）的依据。

#### 4.5.2 Robot Context + Scene Blackboard：运行时观察与对象 grounding

- `/scene_state` blackboard（§5.3）负责把自然语言名词 grounding 到持久 `object_id`，LLM 通过 `scan_scene` 获得 id 清单。
- `get_robot_context()` 返回只读运行时快照：
  - **场景摘要**：新鲜度（`fresh` / `age_sec`）、物体清单（id / label / confidence / 是否在工作空间内，不含坐标）
  - **依赖 readiness**：感知服务、pick/place action、MoveIt、夹爪是否在线，汇总为 `ready_for_pick_place`
  - **历史**：`last_error` + `recent_events`（最近 10 条工具事件）
  - **`recommended_next_step`**：桥对当前状态的软建议
- LLM 在新任务开始与错误恢复时调用，作为重规划的观察输入。

#### 4.5.3 Plan Executor + Structured Feedback：计划校验与失败反馈

- `execute_plan(plan_json)` 两阶段：**先校验**（每步工具必须在 action library 注册、参数齐全，否则整计划拒绝且不执行任何一步）、**再顺序执行**。
- 任一步返回 `failed` / `aborted` 时立即停止，返回结构包含：
  - `failed_step_index`：停在哪一步
  - `steps[]`：已执行各步的完整 Observation（工具、参数、结果）
  - 失败步的三字段错误（code / reason / suggestion，§4.3）
  - `final_context`：停止时刻的 robot context 快照
- **失败反馈 → 重规划闭环**：LLM 拿到"停在哪、什么错误码、当时世界什么样"，可以产出修补计划（如先 `scan_scene` 再续执行剩余步骤），而不是从头重试。范式取舍见 D6。
...
### D6 计划级执行（execute_plan）与失败反馈重规划

- **选择**：在逐步 ReAct（D1）之上增加 `execute_plan`：LLM 一次提交短线性计划，桥内逐步执行，任一步 failed/aborted 立即停止，返回 `failed_step_index` + 每步 Observation + `final_context`（§4.5.3）。同时新增 `get_action_library` / `get_robot_context` 两个只读观察工具。
- **理由**：简单任务一次规划省 token 与往返；失败时的结构化反馈（停在哪一步、什么错误码、停止时刻的上下文）让 LLM 做**针对性重规划**，而不是从头 ReAct。这是课设工作量与通用性论点（§1.3）的核心展示。
- **边界**：计划是**短线性序列**——不支持条件分支、循环，也不支持"用上一步 scan 结果填后续参数"（所有 id 必须提交前已知）。需要分支/循环/中途决策的任务（§3.5 的 S2、S3、S5）退回逐步 ReAct。两种范式并存：简单任务走 `execute_plan`，复杂拆解走逐步调用。
- **升级路径**：计划 DSL 加条件/循环；或参考 Code-as-Policies 让 LLM 生成受限代码。
